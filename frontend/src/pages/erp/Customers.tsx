                                    ...
                                    <td style={tdStyle}>
                                        <button
                                            onClick={async () => {
                                                ...
                                                await fetchJson(`/erp/customers/${c.id}`, { method: 'PATCH', body: JSON.stringify({ status: newStatus }) });
                                                queryClient.invalidateQueries({ queryKey: ['erp-customers'] });
                                            }}
                                            style={{
                                            ...
                                            title={c.status === 'active' ? (isChinese ? '点击停用' : 'Click to disable') : (isChinese ? '点击启用' : 'Click to enable')}
                                        >
                                            <span style={{ width: 6, height: 6, borderRadius: '50%', background: c.status === 'active' ? '#22c55e' : '#ef4444', flexShrink: 0 }} />
                                            {c.status === 'active' ? (isChinese ? '启用' : 'Active') : (isChinese ? '停用' : 'Inactive')}
                                        </button>
                                    </td>
