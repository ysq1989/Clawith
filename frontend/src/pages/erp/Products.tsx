/**
 * Products — Product management page with search, category filter, low-stock highlighting, and CRUD.
 */
...
function ProductForm({
    product, categories, onClose, isChinese,
}: {
    product?: Product;
    categories: string[];
    ...
    const handleSubmit = async () => {
        if (!form.name.trim()) { setError(isChinese ? '请输入产品名称' : 'Please enter product name'); return; }
        ...
        <div style={{ position: 'fixed', inset: 0, zIndex: 10000, background: 'rgba(0,0,0,0.5)', display: 'flex', alignItems: 'center', justifyContent: 'center' }} onClick={() => onClose(false)}>
            <div style={{ background: 'var(--bg-primary)', borderRadius: 12, border: '1px solid var(--border-subtle)', width: 520, maxHeight: '90vh', overflow: 'auto', padding: 24, boxShadow: '0 20px 60px rgba(0,0,0,0.3)' }} onClick={e => e.stopPropagation()}>
                <h3 style={{ margin: '0 0 20px', fontSize: 16, fontWeight: 600, color: 'var(--text-primary)' }}>
                    {product ? (isChinese ? '编辑产品' : 'Edit Product') : (isChinese ? '新建产品' : 'New Product')}
                </h3>
                <div style={{ display: 'flex', flexDirection: 'column', gap: 14 }}>
                    <div style={{ display: 'flex', gap: 12 }}>
                        <div style={{ flex: 1 }}>
                            <FormField label={isChinese ? '产品名称 *' : 'Product Name *'} value={form.name} onChange={v => update('name', v)} />
                        </div>
                        <div style={{ flex: 1 }}>
                    ...
                    <div style={{ display: 'flex', gap: 12 }}>
                        <div style={{ flex: 1 }}>
                            ...
                            <input
                                list="erp-product-categories"
                                value={form.category}
                                onChange={e => update('category', e.target.value)}
                                style={{ ...inputStyle, width: '100%' }}
                            />
                            <datalist id="erp-product-categories">
...
export default function Products() {
    ...
    const { data, isLoading } = useQuery({
        queryKey: ['erp-products', search, categoryFilter, page],
        queryFn: () => fetchJson<ProductsResponse>(
            `/erp/products?search=${encodeURIComponent(search)}&category=${encodeURIComponent(categoryFilter)}&page=${page}&page_size=20`,
        ),
    });

    const deleteMutation = useMutation({
        mutationFn: (id: string) => fetchJson(`/erp/products/${id}`, { method: 'DELETE' }),
    ...
    const handleDelete = async (id: string) => {
        const ok = await dialog.confirm(
            isChinese ? '确定删除此产品？' : 'Are you sure you want to delete this product?',
            { title: isChinese ? '删除产品' : 'Delete Product', danger: true, confirmLabel: isChinese ? '删除' : 'Delete' },
        );
            ...
            <div style={{ display: 'flex', gap: 12, alignItems: 'center', flexWrap: 'wrap' }}>
                <div style={{ position: 'relative', flex: '1 1 240px', maxWidth: 360 }}>
                    ...
                    <input
                        ...
                        placeholder={t('erp.products.searchPlaceholder', '搜索产品名称、SKU...')}
                ...
                <div style={{ flex: 1 }} />
                <button style={btnPrimary} onClick={() => { setEditingProduct(undefined); setShowForm(true); }}>
                    <IconPlus size={16} stroke={2} />
                    {t('erp.products.new', '新建产品')}
                </button>
            </div>
                                    ...
                                    <tr key={p.id} style={{ borderBottom: '1px solid var(--border-subtle)', background: rowBg }}>
                                        ...
                                        <td style={tdStyle}>
                                            <span style={{
                                                ...
                                                background: p.status === 'active' ? 'rgba(34,197,94,0.12)' : 'rgba(239,68,68,0.12)',
                                                border: `1px solid ${p.status === 'active' ? 'rgba(34,197,94,0.3)' : 'rgba(239,68,68,0.3)'}`,
                                                color: p.status === 'active' ? '#22c55e' : '#ef4444',
                                            }}>
                                                {p.status === 'active' ? (isChinese ? '在售' : 'Active') : (isChinese ? '停售' : 'Inactive')}
                                            </span>
                                        </td>
                                        <td style={{ ...tdStyle, textAlign: 'center' }}>
                                            <div style={{ display: 'flex', gap: 6, justifyContent: 'center' }}>
                                                <button onClick={() => { setEditingProduct(p); setShowForm(true); }} style={{ background: 'none', border: '1px solid var(--border-subtle)', borderRadius: 4, padding: '3px 6px', color: 'var(--text-tertiary)', cursor: 'pointer', display: 'flex' }} title={isChinese ? '编辑' : 'Edit'}>
                                                    <IconEdit size={14} stroke={1.5} />
                                                </button>
                                                <button onClick={() => handleDelete(p.id)} style={{ background: 'none', border: '1px solid var(--border-subtle)', borderRadius: 4, padding: '3px 6px', color: 'var(--text-tertiary)', cursor: 'pointer', display: 'flex' }} title={isChinese ? '删除' : 'Delete'}>
