...
export default function XHSDashboard() {
    ...
    const stats = overview || { total_views: 0, total_likes: 0, total_comments: 0, total_bookmarks: 0, note_count: 0, content_status: {} };

    return (
        <div style={{ maxWidth: 1200 }}>
            {/* Stat cards */}
            <div style={{ display: 'grid', gridTemplateColumns: 'repeat(4, 1fr)', gap: 16, marginBottom: 24 }}>
                <StatCard icon={<IconEye size={22} />} label={t('xhs.dashboard.totalViews', '总曝光')} value={stats.total_views} color="#3b82f6" />
                <StatCard icon={<IconHeart size={22} />} label={t('xhs.dashboard.totalLikes', '总点赞')} value={stats.total_likes} color="#ff2442" />
                <StatCard icon={<IconMessage size={22} />} label={t('xhs.dashboard.totalComments', '总评论')} value={stats.total_comments} color="#f59e0b" />
                <StatCard icon={<IconBookmark size={22} />} label={t('xhs.dashboard.totalBookmarks', '总收藏')} value={stats.total_bookmarks} color="#10b981" />
            </div>
